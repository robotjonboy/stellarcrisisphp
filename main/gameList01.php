...
function gameList($vars)
{
	...
	$fields = 'series.*, games.id AS game_id, players.id AS player_id, players.ended_turn';
	...
	while ($row = mysql_fetch_array($select))
		...
		$series_description_link = '<a style="text-decoration: none; color: red;" href="javascript:void(0)" '.$popup.'>'.$row['name'].'</a>';
								   ($row['creator'] != 'admin' ? '<div style="font-size: 7pt;">Created by '.$row['creator'].'</div>' : '');
	
		$player_listing = '<span class="playerListing">'.
						  '<a href="javascript:void(0)">[&nbsp;list<span>'.playerList($row['game_id'], $empire).'</span>&nbsp;]</a></span>';
		...
		$update_readiness = '<span class='.($row['ended_turn'] ? 'greenBold>' : 'redBold>not ').' ready</span>';
...
<?
	echo drawButtons($empire).serverTime().
		 '<div style="margin-top: 10pt; font-size: 8pt; text-align: center;"><a href="open_games.rss"><img src="/server_images/rss.gif" style="vertical-align: middle;" border=0></a> Open games&nbsp;&nbsp;'.
		 '<a href="updates.rss?id='.md5($empire['id'].$empire['name'].$empire['join_date']).'"><img src="/server_images/rss.gif" style="vertical-align: middle;" border=0></a> Your updates</div>'.
...
?>
	<tr>
		<td colspan=6 style="color: white; text-align: center;">
			...
			<div style="font-size: 13pt;">Invitations</span>
			<div style="font-size: 8pt;">Click on a series name for a full list of its parameters.</span>
...
function playerList($gameID, $empire)
{
	...
	if ($select->num_rows > 10)
		...
		while ($row = mysql_fetch_array($select))
			...
			$player_list .= '<div style="width=50; padding: 2pt; font-size: 8pt; text-align: left">'.$row['name'].'</div>';
		...
		while ($row = mysql_fetch_array($select))
			...
			if (($row['map_visible'] or $row['closed']) and $current_team != $row['team'])
				{
				$current_team = $row['team'];
				$player_list .= '<div style="background-color: #003; width=50; padding: 2pt; font-size: 8pt; text-align: left">'.$row['name'].'</div>';
...
function onlinePlayers()
{
	...
	if ($count = $select->num_rows)
		...
		$list = '<div style="text-align: center; margin-top: 10pt;">'.
				...
				'<option value=0 selected>'.$count.' player'.($count != 1 ? 's' : '').' currently online...'.
				'<option value=0>';
