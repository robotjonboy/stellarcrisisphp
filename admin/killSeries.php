...
function killSeries_processing($vars)
{
	...
	if ($vars['killSeries'])
		...
		sc_query('DELETE messages.* FROM '.$tables.' WHERE series.id = '.$series['id']);

		$tables = 'series INNER JOIN games ON series.id = games.series_id '.
						 'INNER JOIN players ON games.id = players.game_id '.
						 'INNER JOIN scouting_reports ON players.id = scouting_reports.player_id';
		sc_query('DELETE scouting_reports.* FROM '.$tables.' WHERE series.id = '.$series['id']);

		// Delete bridier data for ongoing games.
		$tables = 'series INNER JOIN games ON series.id = games.series_id '.
						 'INNER JOIN bridier ON games.id = bridier.game_id';
		sc_query('DELETE bridier.* FROM '.$tables.' WHERE series.id = '.$series['id']);

		$tables = 'series INNER JOIN games ON series.id = games.series_id '.
						 'INNER JOIN history ON games.id = history.game_id';
		sc_query('DELETE history.* FROM '.$tables.' WHERE series.id = '.$series['id']);

		sc_query('DELETE FROM diplomacies WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM explored WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM fleets WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM games WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM invitations WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM players WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM ships WHERE series_id = '.$series['id']);
		sc_query('DELETE FROM systems WHERE series_id = '.$series['id']);

		sc_query('DELETE FROM series WHERE id = '.$series['id']);
