...
function buildScoutingReport($series, $game, $system, $recipient)
{
	...
	$report = '<span'.$system_name_color.'>'.$system_name.' ('.$local_coordinates.')</span>'.
			  $owner.
			  '<table border=0 cellpadding=0 cellspacing=0 style="font-size: 8pt; margin-left: auto; margin-right: auto; margin-top: 10pt;">'.
			  '<tr class=center><td colspan=3>'.(in_array($x.','.($y+1), $jumps) ? '<img src="images/vert.gif">' : '').'</td></tr>'.
			  '<tr><td>'.(in_array(($x-1).','.$y, $jumps) ? '<img src="images/horz.gif">' : '').'</td>'.
			  '<td><table border=0 cellspacing=0 cellpadding=5 style="font-size: 8pt;">'.
			  '<tr class=center>'.
			  '<td'.$system_name_color.'>'.$system['mineral'].'<br>'.$system['agriculture'].'<br>?</td><td>'.$icon.'</td>'.
			  '<td'.$system_name_color.'>'.$system['fuel'].'<br>'.($system['population']+$population_adjustement).'<br>?</td></tr>'.
			  '<tr class=center><td colspan=3'.$system_name_color.'>'.$system_name.' ('.$local_coordinates.')</td></tr>'.
			  '</table></td><td valign=middle>'.(in_array(($x+1).','.$y, $jumps) ? '<img src="images/horz.gif">' : '').'</td></tr>'.
			  '<tr class=center><td colspan=3>'.(in_array($x.','.($y-1), $jumps) ? '<img src="images/vert.gif">' : '').'</td></tr></table>';
	...
	if (count($ship_inventory))
		...
		foreach (array_keys($ship_inventory) as $owner)
			...
			foreach (array_keys($ship_inventory[$owner]) as $ship_type)
				$line .= '<span class=white>'.$ship_type.'</span> ('.$ship_inventory[$owner][$ship_type].') ';
...
function scoutingScreen($vars)
{
	...
	if (!count($vars['scouting_systems']))
		...
		if (mysql_num_rows($select))
			...
			if ($server['local_coordinates'])
				$report .= ' Coordinates will be translated to the recipient\'s coordinate space.';

			$recipient_list = '<option>'.implode('<option>', $missive_recipients);
			}
		else
			$report = '<div class=center>The following is the scouting report you selected, however '.
					  'you haven\'t met anyone else yet so it cannot be sent.';

		$report .= '</div><div style="margin-top: 10pt;"><table cellpadding=5 border=0 style="margin-left: auto; margin-right: auto;">';
		...
		foreach ($vars['scouting_systems'] as $coordinates)
			...
			$tmp_reports[] = '<tr><td valign=top align=center rowspan=2>'.
							 '<input type=checkbox checked name="scouting_systems[]" value="'.$coordinates.'"></td>'.
							 '<td style="text-align: left;">'.$data.'</td>'.
							 '<td valign=top style="text-align: left;">Your comments:<p>'.
							 '<textarea align=top name="notes:'.$coordinates.'" rows=5 cols=40></textarea></td></tr>'.
							 '<tr><td colspan=2 style="text-align: left;">'.$ships.'</td></tr>';
			}
...
function scoutingScreen_processing($vars)
{
	...
	if (!count($vars['scouting_systems']))	return sendGameMessage($player, 'No planets included in report. Report cancelled.');
	if (!count($vars['missive_to']))		return sendGameMessage($player, 'No recipients selected. Report cancelled.');
	...
	foreach ($vars['missive_to'] as $recipient_name)
		...
		foreach ($vars['scouting_systems'] as $observation)
			...
			list($data, $ships) = buildScoutingReport($series, $game, $system, $recipient);

			$comment = sanitizeString($vars['notes:'.$observation]);
			$comment = str_replace("\n", '<br>', $comment);

			$observations[] = '<tr><td style="text-align: left;">'.$data.'</td>'.
							  '<td valign=top style="text-align: left;"><span class=messageHeader>'.$player['name'].'\'s comments:</span>'.
							  '<p><span class=messageTextPrivate>'.$comment.'</span></td></tr>'.
						      '<tr><td colspan=2 style="text-align: left;">'.$ships.'</td></tr>';
			...
			if (!mysql_num_rows($select))
				...
				// could have happenned since the coordinate is supposed to come from the systems screen. An interrupted form load?
				// Anyway, this caused an infinite loop in both the mini-map and map screens. Let's check for it until we know more.
				...
				if (strlen($system['owner']))	$values[] = 'owner = "'.$system['owner'].'"';
		...
		$message = '<table cellspacing=5>'.implode('<tr><td colspan=2><hr></td></tr>', $observations).'</table>';
		
		sendPlayerMissive($player, $recipient['id'], implode(', ', $vars['missive_to']), 'scout', $message);
		}

	return sendGameMessage($player, 'Scouting report sent to '.implode(', ', $vars['missive_to']));
}
?>